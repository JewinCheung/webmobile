...
const wxUtils = {
    ...
    setWXCfg(appId, timestamp, nonceStr, signature) {
        var wxObj = {
            ...
            jsApiList: ["getNetworkType", "chooseImage", "previewImage",  "uploadImage", "scanQRCode", "getLocation", "checkJsApi", "openLocation","getLocalImgData"]
        };
        if (localStorage) {
    ...
    getLocalImg: function (imgId,success) {
        wx.getLocalImgData({
            localId: imgId, // 图片的localID
            success: function (res) {
                 var localData = res.localData; // localData是图片的base64数据，可以用img标签显示
                 if (typeof (success) == "function") {
                    success(res.localData);
                }
            }
        });
    },
