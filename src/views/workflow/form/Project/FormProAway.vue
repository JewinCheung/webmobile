<template>
    <div>
        ...
        <group>
            ...
            <cell title="出差日期">{{postJSON.PreSDate | substr(10)}}</cell>
            <cell title="返回日期">{{postJSON.PreEDate | substr(10)}}</cell>
            <cell title="出差目的">{{postJSON.Target}}</cell>
            <cell title="出差目的">{{postJSON.Result}}</cell>
            <cell title="总计金额">{{postJSON.AllMoney}}</cell>
            <cell title="备注" :inline-desc="postJSON.Memo"></cell>
        </group>
        <divider>出差费用明细</divider>
        <!--物品申请-->
